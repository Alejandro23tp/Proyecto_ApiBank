...
class CreateTriggerAfterPrestamosParticipanteInsertOrUpdate extends Migration
{
    /**
    ...
    public function up()
    {
        ...
        DB::unprepared('
            CREATE OR REPLACE FUNCTION after_prestamos_participante_insert_or_update()
            RETURNS TRIGGER AS $$
            DECLARE
                total_prestamo DECIMAL(10, 2);
                total_interes DECIMAL(10, 2);
            BEGIN
                -- Calcular la suma total de "pp_prestamo" para la "pp_semana" de la fila agregada o editada
        ...
        // Crear trigger
        DB::unprepared('
            CREATE TRIGGER after_prestamos_participante_insert_or_update
            AFTER INSERT ON prestamos_participante
            FOR EACH ROW
            EXECUTE FUNCTION after_prestamos_participante_insert_or_update();
        ');
    }
    ...
    {
        DB::unprepared('
            DROP TRIGGER IF EXISTS after_prestamos_participante_insert_or_update ON prestamos_participante;
        ');
        DB::unprepared('
            DROP FUNCTION IF EXISTS after_prestamos_participante_insert_or_update();
        ');
    }
}
